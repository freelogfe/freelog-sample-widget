...
async function handleDownloadCard() {
  ...
  try {
    ...
    // 移动端：优先使用 Web Share API，用户可在分享页选择「保存图片」存到相册
    if (isMobile.value && navigator.share && navigator.canShare?.({ files: [file] })) {
      try {
        await navigator.share({
          files: [file],
          title: data.nodeInfo.nodeTitle || "二维码名片"
        });
        showToast("已分享，可保存到相册");
      } catch (shareErr: any) {
        // 用户取消分享时 fallback 到下载
