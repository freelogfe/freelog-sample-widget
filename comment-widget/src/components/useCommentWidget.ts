...
export function useCommentWidget(props: CommentWidgetProps) {
  ...
  function inferUserRoleFromExhibit(
    ...
    if (
      articleUserId != null &&
      !Number.isNaN(Number(articleUserId)) &&
      Number(articleUserId) === u
    ) {
      return "creator";
    }
  ...
  function mapThreadToComment(
    ...
    const recipientUserId =
      item.recipientInfo?.userId ?? (replyTo ? findUserIdInThread(root, replyTo) : undefined);
  ...
  function checkVerticalRootLoadMore() {
    ...
    const anchor =
      rootLoadMoreSentinelRef.value ??
      commentListRef.value?.lastElementChild ??
  ...
  function isBlockedForNodeAdminInteraction(comment: Comment, parentComment?: Comment): boolean {
  ...
  const toggleMoreMenu = (commentId: string, event: MouseEvent) => {
    ...
    if (showMoreMenu.value === commentId) {
      ...
      const openAbove =
        spaceBelow < estimatedH && (spaceAbove >= estimatedH || spaceAbove > spaceBelow);
  ...
  function bindParentScrollListeners() {
    ...
    try {
      ...
      cleanups.push(() =>
        scrollTarget.removeEventListener("scroll", onParentScrollOrViewportChange)
      );
